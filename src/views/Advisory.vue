<template>
    <div class="detail">
            ...
            <v-layout column fill-height>
                        ...
                        <v-layout align-center justify-space-between row fill-height>
                            ...
                            <v-flex xs6>
                                <img
                                        class="avatar"
                                        :src="userInfo.avatar"
                                        style="width:40%; float:right; margin:0 20% 10px 0 ;"
                                >
                            </v-flex>

                            <v-flex xs6>
                                <!--昵称-->
                                <v-flex xs1 style="max-width:100% !important;">
                                    <v-container fill-height>
                                        <v-layout align-left justify-center row fill-height>
                                            <v-flex shrink>
                ...
                <v-flex xs6>
                    <v-container
                        ...
                        >简介：{{ userInfo.desc.length > 60? userInfo.desc.substring(0, 60) + '...': userInfo.desc }}</span>
                            </v-container>
                        </v-flex>
                    </v-container>
                </v-flex>
              ...
              >{{ (userInfo.follow).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</span>
                            <br>
                            <span class="font--text">关注</span>
                        </v-flex>

                        <v-flex xs6 style="width:30%;margin-right:20%; text-align:center;">
              <span
                      class="headline font-weight-bold"
              >{{ (userInfo.fans).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</span>
                            <br>
        ...
        <v-layout>
            <v-flex>
                <v-tabs fixed-tabs>
                    <v-tab v-for="tab in ['付费咨询']" :key="tab">{{ tab }}</v-tab>
                    <!--公开咨询-->
                    <!--<v-tab-item :key="'公开咨询'">-->
                    ...
                    <v-tab-item :key="'付费咨询'">
                        <v-card flat min-height="250">
                            ...
                            <v-flex xs12 style="display: flex;align-items: center;justify-content: center;">
                                <v-btn
                                        color="info"
                                        @click="$router.push({name:'advisory-publish',query:{id:userId}})"
                                >发起付费咨询
                                </v-btn>
                            </v-flex>
                            <v-divider></v-divider>
                            ...
                            <v-container>
                                <v-layout row wrap style="line-height: 1.5;width:100%;">
                                    ...
                                    >
                                        ...
                                        <h4 style="text-align: right">提问日期：{{value.time}} <br> 状态：<span
                                                style="color: red">{{value.state}}</span></h4>
                                    </v-card>
    ...
    export default {
        ...
        data() {
            return {
                ...
                message: [{title: "加载中..", subtitle: "发表文章", time: "1天前"}], // 动态
                answers: [], // 回答
                articles: [
